...
class World(metaclass=AutoWorldRegister):
    ...
    def custom_hints_data_populate(self,
                                    populate_region_area_map = True,
                                    populate_location_region_map = True,
                                    populate_item_score_map = True,
                                    region_area_map_condensers = [],
                                    controls = {}) -> None:

        if populate_region_area_map:
            for region in set([location.parent_region.name for location in self.multiworld.get_locations(player=self.player) if location.item.code]):
                area = region
                for condenser in region_area_map_condensers:
                    area = area.split(condenser)[0]
                self.multiworld.custom_hints.add_region(self.player, region, area)
        
        if populate_location_region_map:
            for location in [location for location in self.multiworld.get_locations(player=self.player) if location.item.code]:
                self.multiworld.custom_hints.add_location(location)
        
        if populate_item_score_map:
            for item in [location.item for location in self.multiworld.get_locations(player=self.player) if location.item.code]:
                self.multiworld.custom_hints.add_item_score(item)

        if 'excluded_areas' in controls:
            self.multiworld.custom_hints.exclude_areas(controls['excluded_areas'])
        if 'excluded_items' in controls:
            self.multiworld.custom_hints.exclude_items([location.item for location in self.multiworld.get_locations()
                                                               if location.item.player == self.player and location.item.name in controls['excluded_items']])
        if 'unhintable_items' in controls:
            self.multiworld.custom_hints.add_unhintable_items([location.item for location in self.multiworld.get_locations()
                                                               if location.item.player == self.player and location.item.name in controls['unhintable_items']])
        if 'goal_items' in controls:
            self.multiworld.custom_hints.add_goals([(location,'Item') for location in self.multiworld.get_locations()
                                                    if location.item.player == self.player and location.item.name in controls['goal_items']])
        if 'goal_locations' in controls:
            self.multiworld.custom_hints.add_goals([(location,'Location') for location in self.multiworld.get_locations()
                                                    if location.item.player == self.player and location.name in controls['goal_locations']])   
    #####################################################################################################
    # AptMarsh - END                                                                                    #
    #####################################################################################################
